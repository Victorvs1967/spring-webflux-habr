package com.vvs.springwebfluxhabr;
...
@SpringBootTest
class SpringWebfluxHabrApplicationTests {

	@Test
	void simpleFluxExample() {
		Flux<String> fluxColor = Flux.just("-> red", "-> green", "-> blue");
		fluxColor.subscribe(System.out::println);
	}

	@Test
	void mapExample() {
		Flux<String> fluxColor = Flux.just("-> red", "-> green", "-> blue");
		fluxColor.map(color -> color.charAt(3)).subscribe(System.out::println);
	}
	...
	void onErrorExample() {
		Flux<String> fluxCalc = Flux.just(-1, 0, 1)
			...
			fluxCalc.subscribe(value -> System.out.println("Next: " + value), error -> System.err.println("Error: " + error));
	}
